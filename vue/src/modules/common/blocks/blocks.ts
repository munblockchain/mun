...
async function getTx(apiCosmos: string, apiTendermint: string, encodedTx: string) {
...
async function decodeTx(apiCosmos: string, apiTendermint: string, encodedTx: string) {
  let fullTx
  let retries = 0
...
export default {
  ...
  getters: {
    getBlocks: (state: { blocks: any }) => (howmany: number | undefined) => {
      return [...state.blocks]
        .sort((a, b) => b.height - a.height)
        .slice(0, howmany)
    },
    getBlockByHeight: (state: { blocks: any[] }) => (height: any) => {
      return state.blocks.find((x: { height: any }) => x.height == height) || {}
    }
  },
  mutations: {
    ADD_BLOCK(state: { blocks: any[]; size: number }, block: any) {
      state.blocks.push(block)
      if (state.blocks.length > state.size) {
        state.blocks.shift()
      }
    },
    RESET_STATE(state: { blocks: never[] }) {
      state.blocks = []
    },
  ...
  actions: {
    init({ dispatch, rootGetters }) {
      if (rootGetters['common/env/client']) {
        rootGetters['common/env/client'].on('newblock', (data: any) => {
          dispatch('addBlock', data)
        })
      }
    },
    async getShiftedBlock({ rootGetters }: any, blockHeight: string) {
      try {
    ...
    async addBlock({ commit, rootGetters }: any, blockData: { data: { value: { block: { header: { height: string; time: any }; data: { txs: any[] } } } } }) {
      try {
        const blockDetails = await axios.get(
          rootGetters['common/env/apiTendermint'] +
            '/block?height=' +
